...
echo "$($PSQL "SELECT winner_team.name AS name FROM games LEFT JOIN teams AS winner_team ON games.winner_id = winner_team.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT opponent_team.name AS name FROM games LEFT JOIN teams AS opponent_team ON games.opponent_id = opponent_team.team_id WHERE year = 2014 AND round = 'Eighth-Final'")"
